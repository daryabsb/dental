          ...
          <div class="card-body">
            ...
            <div class="table-responsive table-wrapper-scroll-y my-custom-scrollbar">
              ...
              <table id="datatable" class="table">
                ...
                <tbody>
					<!-- <perfect-scrollbar> -->
                  <tr v-for="treat in getTreatments" :key="treat.id">
                    <td>
                      <a href="../hospital/patient-profile.html"
                        ><img
                          src="~assets/images/users/user-10.jpg"
                          alt=""
                          class="thumb-sm rounded-circle mr-2"
                        />{{patientName(treat.patient)}}</a
                      >
                    </td>
                    <td>{{treat.description}} [<span v-for="file in treat.files" :key="file.id">
						<a class="text-success" @click="openPdf(file.file)">{{file.filename}}</a>
...
export default {
  ...
  methods: {
	  patientName(id) {
		  let patient = this.$store.state.patients.find(patient=>patient.id===id)
		  return patient.name;
	  },
	  openPdf (link) {
		   this.link = link;
