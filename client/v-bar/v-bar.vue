...
export default {
  ...
  methods: {
    ...
    getBarInternal(axis) {
      ...
      internalSize =
        (positionWrapper / (sizeWrapper - sizeContainer)) *
        (sizeContainer - sizeBar);
    ...
    getCoordinates(e, axis) {
      ...
      if (axis === 'X') {
        ...
        sizeContainer = this.container.scrollHeight;
        offsetContainer =
          this.container.elm.offsetTop - this.bars.vertical.size * 1.4;
        clientDirection = e.clientY - this.dragging.offset;
      }
      coordinate =
        ((sizeWrapper - sizeContainer) * (clientDirection - offsetContainer)) /
        (sizeContainer - sizeBar);
    ...
    startDrag(e) {
      ...
      if (axis === 'Y') {
        if (dataDrag === 'bar') {
          elementOffset =
            e.explicitOriginalTarget.offsetTop + this.bars.vertical.size * 1.4;
    ...
    getSizes(X, Y) {
      ...
      this.bars.horizontal.size =
        this.wrapperObj.scrollWidth - this.container.scrollWidth > 24 &&
      ...
      this.bars.vertical.size =
        this.wrapperObj.scrollHeight - this.container.scrollHeight > 24 &&
