				...
				<b-card style="height:44.3rem;">
					...
					>
						...
						<template #cell(name)="row">
							<div draggable="true" @dragstart="onEventDragStart($event, row.item)">
													...
													<b-col md="6">
														<b-card-body :title="selectedPatient.name">
															<b-card-text>
																...
																</p>This is a wider card with supporting text as
									...
									<b-row>
										<b-col v-if="selectedPatient">
											<div class="accordion" role="tablist">
												...
												<b-card no-body class="mb-1">
													<b-card-header header-tag="header" class="p-1" role="tab">
														<b-button block v-b-toggle.accordion-1 variant="info">Medical Examinations</b-button>
													</b-card-header>
													<b-collapse id="accordion-1" accordion="my-accordion" role="tabpanel">
														<!-- visible -->
														<b-card-body>
															<medical-alias :medicals="selectedPatient.medicals"></medical-alias>
														</b-card-body>
													</b-collapse>
												</b-card>
												<b-card no-body class="mb-1">
														...
														<b-card-body class="treatments-activity">
															<b-card-text>
																<b-list-group>
																	...
																	v-for="visit in patientVisits(selectedPatient.id)"
																	:key="visit.id">{{visit.description ? visit.description : '-'}}
																	<b-badge variant="primary">{{ $moment(visit.date).format('yyyy-MM-DD') }} </b-badge>
																	</b-list-group-item>
																</b-list-group>
																<!-- <pre>{{ patientVisits(selectedPatient.id) }}</pre> -->
															</b-card-text>
														</b-card-body>
													</b-collapse>
												</b-card>
										...
										<b-col v-if="selectedPatient">
											<h4>
												...
												<template v-else>Mrs</template>
												{{selectedPatient.name}}'s treatments
											</h4>
											<clinical-alias :examinations="selectedPatient.examinations"></clinical-alias>
...
export default {
	...
	methods: {
		...
		onEventClick(event, e) {
			...
			this.selectedPatient = this.patientsData.results.find(
		...
		onEventDrop({ event, originalEvent, external }, deleteEvent) {
			...
			if (external) {
				let data = {
					...
					title: event.name,
					description: "",
					date: this.$moment(event.startDate).format(
						"yyyy-MM-DDTHH:mm"
					),
				};
				...
				let data = {
					...
					date_to: this.$moment(event.endDate).format(
						"yyyy-MM-DDTHH:mm"
					),
				};
		...
		onEventDurationChange(event) {
			...
			let data = {
				...
				date: this.$moment(event.event.startDate).format(
					"yyyy-MM-DDTHH:mm"
				),
				date_to: this.$moment(event.event.endDate).format(
					"yyyy-MM-DDTHH:mm"
				),
			};
			this.$store.dispatch("editAppointment", data);
		...
		patientVisits(id) {
			...
			const pApps = this.getAllAppointments
				.filter((appt) =>					
					this.$moment(appt.date).format(formats)<=this.$moment(today).format(formats)
					)
	...
	watch: {
		calEvents() {
			...
			this.getAppointments.results.forEach((app) => {
				let evt = {};
				let patient = this.patientsData.results.find(
					(p) => p.id === app.patient
				);
				evt.id = app.id;
				evt.patient = app.patient;
